...
class RoomService:
    ...
    @classmethod
    async def get_rooms(cls, db: AsyncSession, rooms_ids: Optional[int], limit: int, offset: int) -> List[models.Room]:
        """Получение списка всех аудиторий"""

        logger.debug("Запрос на получение списка аудиторий")

        rooms = await RoomDBService.get_rooms(db=db, rooms_ids=rooms_ids, limit=limit, offset=offset)
